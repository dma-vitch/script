...
FILE_NAME=${NMS_SERVER_NAME}_db_${DATE}.dump
FILE_NAME_TGZ=${FILE_NAME}.tgz
FILE_NAME_CONFIG=zabbix_config_${DATE}
FILE_NAME_CONFIG_TGZ=${FILE_NAME_CONFIG}.tgz
CONFIG_LIST="/etc/httpd/conf.d/zabbix.conf /etc/zabbix /usr/lib/zabbix /usr/share/zabbix /etc/php-fpm.d /etc/nginx/conf.d/zabbix.bss.ural.mts.ru.conf /etc/php.ini"
...
create_archive()
{
    ...
    echo "Creating Archive Configs."
    tar cvzf ${FILE_NAME_CONFIG_TGZ} ${CONFIG_LIST}

    echo "Setting Owner."
    chown $USER:$USER ${FILE_NAME}.tgz
    chown $USER:$USER ${FILE_NAME_CONFIG}.tgz

    echo "Removing Unarchived Backup."
    rm $BACKUP_DIR/${FILE_NAME}

    echo "Removing Archives older than 3 days."
    rm $BACKUP_DIR/${NMS_SERVER_NAME}_db_$PREV*
    rm $BACKUP_DIR/zabbix_config_$PREV*
